...
cond_csv_name <- paste0(condition_column_names, "_all_dthplce_csv_generator_for_all_tissues")
...
for (folder in subfolders) {
  ...
  boxplot_data <- final_data %>%
    ...
    # filter(DTHPLCE %in% c("Hospital inpatient", "Emergency room")) %>% # Filter out if we want to have all tissues not just comparing HI to ER
