...
#This code currently gets the log2 TMM expression of DDIT3, then centers and scales it, and then compared the pc2 score (which also gets centerd and scaled) for each tissue

# Uncommented out many of the "print" plots

#Sub in DDIT3 for DDIT3 (or CHOP or DDIT3)
...
for (folder in subfolders) {
  ...
  exprs_DDIT3 <- exprs_ISR_scaled %>%
    select(SAMPID, DDIT3)
  
  # # Join with subject phenotypes
  exprs_DDIT3 <- exprs_DDIT3 %>%
    full_join(annotations_sample_attributes_filtered, by = "SAMPID") %>%
    select(DDIT3, SAMPID, AGE, SEX) %>%
    unique() %>%
    na.omit()
  #
  # # Join with subject phenotypes
  exprs_DDIT3_tissue <- exprs_DDIT3 %>%
    full_join(annotations_sample_attributes_filtered, by = "SAMPID") %>%
    select(DDIT3, SMTSD, SAMPID) %>%
    unique() %>%
    na.omit()
  ...
  loadings_readin <- file.path(paste0(gtex_input, "/FacLoads_12_factors.csv")) # includes DDIT3, is from folder 2024_09_23
  ...
  boxplot_data <- boxplot_data %>%
    left_join(exprs_DDIT3, by = "SAMPID")
  ...
  # Scatterplot for AGE vs DDIT3
  rho_DDIT3 <- cor.test(boxplot_data$AGE, boxplot_data$DDIT3, method = "spearman")
  adjusted_p_DDIT3 <- p.adjust(rho_DDIT3$p.value, method = "BH")
  
  asterisks_DDIT3 <- ifelse(adjusted_p_DDIT3 < 0.0001, "****",
                            ifelse(adjusted_p_DDIT3 < 0.001, "***",
                                   ifelse(adjusted_p_DDIT3 < 0.01, "**",
                                          ifelse(adjusted_p_DDIT3 < 0.05, "*", "ns"))))
  ...
  results_df <- rbind(results_df, data.frame(Tissue = tissue,
                                             Comparison = "DDIT3",
                                             Spearman_Rho = rho_DDIT3$estimate,
                                             P_Value = adjusted_p_DDIT3,
                                             Asterisks = asterisks_DDIT3))
  ...
  p2 <- ggplot(boxplot_data, aes(x = AGE, y = DDIT3)) +
    geom_point(color = "violet") +
    geom_smooth(method = "lm", color = "red", se = TRUE) +
    annotate("text", x = Inf, y = Inf, label = paste0("rho: ", round(rho_DDIT3$estimate, 3),
                                                      "\nP: ", asterisks_DDIT3),
             hjust = 1.1, vjust = 2, size = 5) +
    ggtitle(paste("AGE vs DDIT3 in", tissue)) +
    theme_minimal()
  ...
  ggsave(filename = paste0(folder_path, "/", last_component, "_AGEvsDDIT3_Scatterplot.png"), plot = p2, width = 10, height = 8, dpi = 300)
  ...
  # Spearman tests for chosen_Factor and DDIT3
  rho_chosen_Factor <- cor.test(boxplot_data$AGE, boxplot_data$chosen_Factor, method = "spearman")
  adjusted_p_chosen_Factor <- p.adjust(rho_chosen_Factor$p.value, method = "BH")
  ...
  rho_DDIT3 <- cor.test(boxplot_data$AGE, boxplot_data$DDIT3, method = "spearman")
  adjusted_p_DDIT3 <- p.adjust(rho_DDIT3$p.value, method = "BH")
  rho_DDIT3$asterisks <- ifelse(adjusted_p_DDIT3 < 0.0001, "****",
                                ifelse(adjusted_p_DDIT3 < 0.001, "***",
                                       ifelse(adjusted_p_DDIT3 < 0.01, "**",
                                              ifelse(adjusted_p_DDIT3 < 0.05, "*", "ns"))))
  ...
  p2_dthplce <- plot_with_spearman_and_dthplce(
    ...
    "DDIT3",
    rho_DDIT3,
    paste(last_component, "AGE vs DDIT3 (color by DTHPLCE)")
  )
  ...
  ggsave(filename = paste0(folder_path_colorcodeDTHPLCE, "/", last_component, "_AGEvsDDIT3_Scatterplot_DTHPLCE.png"), plot = p2_dthplce, width = 10, height = 8, dpi = 300)
  
  
}

# Final plot for Spearman rho
# One plot for chosen_Factor and one for DDIT3
results_chosen_Factor <- results_df[results_df$Comparison == "chosen_Factor", ]
results_DDIT3 <- results_df[results_df$Comparison == "DDIT3", ]
...
p_final_DDIT3 <- ggplot(results_DDIT3, aes(x = Spearman_Rho, y = reorder(Tissue, Spearman_Rho))) +
  geom_point() +
  ggtitle("Spearman Rho for AGE vs DDIT3") +
  theme_minimal()
...
ggsave(filename = paste0(folder_path, "/SpearmanR_AGEvsDDIT3.png"), plot = p_final_DDIT3, width = 10, height = 8, dpi = 300)
...
# DDIT3
p_significant_DDIT3 <- plot_significant_rhos(results_DDIT3, "DDIT3")

# print(p_significant_chosen_Factor)
# print(p_significant_DDIT3)
...
# DDIT3 with Asterisks
p_asterisks_DDIT3 <- plot_with_asterisks(results_DDIT3, "DDIT3")

# print(p_asterisks_chosen_Factor)
# print(p_asterisks_DDIT3)

# Plot 3: Combined Spearman Rho plot for chosen_Factor and DDIT3
combined_data <- merge(results_chosen_Factor, results_DDIT3, by = "Tissue", suffixes = c(chosen_Factor, "_DDIT3"))

colnames(combined_data)[3] <- "Spearman_Rho_chosen_Factor"

p_combined <- ggplot(combined_data) +
  geom_point(aes(x = Spearman_Rho_DDIT3, y = reorder(Tissue, Spearman_Rho_DDIT3)), color = "pink", size = 3) +
  geom_point(aes(x = Spearman_Rho_chosen_Factor, y = reorder(Tissue, Spearman_Rho_DDIT3)), color = "gray", size = 3) +
  ggtitle(paste("Combined Spearman Rho for AGE vs DDIT3 (pink) and", chosen_Factor, "(gray)")) +
  theme_minimal()

print(p_combined)

# Plot 4: Combined Spearman Rho plot for chosen_Factor and DDIT3
combined_data <- merge(results_chosen_Factor, results_DDIT3, by = "Tissue", suffixes = c(chosen_Factor, "_DDIT3"))

colnames(combined_data)[3] <- "Spearman_Rho_chosen_Factor"

p_combined2 <- ggplot(combined_data) +
  geom_point(aes(x = Spearman_Rho_DDIT3, y = reorder(Tissue, Spearman_Rho_chosen_Factor)), color = "pink", size = 3) +
...
ggsave(filename = paste0(folder_path, "/SpearmanR_p_significant_DDIT3.png"), plot = p_significant_DDIT3, width = 10, height = 8, dpi = 300)
...
ggsave(filename = paste0(folder_path, "/SpearmanR_p_asterisks_DDIT3.png"), plot = p_asterisks_DDIT3, width = 10, height = 8, dpi = 300)
